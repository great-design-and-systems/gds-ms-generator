...
function execute(config, callback) {
    ...
    new IterateService(config.containers.services,
        function (service, done) {
            var servicePath = path.join(service.name, 'unix', 'uninstall.sh');
            shCalls += 'sh ' + servicePath + '\n';
            done();
        }, function (err) {
            if (err) {
                callback({
                    message: 'Failed to create uninstall-all scripts'
                });
            } else {
...
function createFile(outputPath, shCalls, callback) {
    fs.writeFile(path.join(outputPath, 'uninstall.sh'), shCalls, callback);
}
