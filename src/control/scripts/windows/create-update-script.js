            ...
            function (service, done) {
                ...
                async.eachSeries([
                    ...
                    function (asyncFunc, callback) {
                        ...
                        shCalls += ' npm run container';
                        createFile(servicePath, shCalls, function (err) {
                            if (err) {
                                callback({
                                    message: 'Failed to create update-service.bat for service ' + service.name
                                })
                            } else {
                                done();
                            }
...
function createFile(outputPath, shCalls, callback) {
    fs.writeFile(path.join(outputPath, 'update-service.bat'), shCalls, callback);
}
